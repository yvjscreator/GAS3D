...
export function PrintZoneInspector({ placement, print, zoneAdjustment, editorMode, onEditorModeChange, onUpdatePrint, onUpdateZone, onAlign }: Props) {
  ...
  return <div className="print-zone-inspector">
    ...
    {print.url ? <div className="upload-preview"><img src={print.url} alt="Arte asignado" /><div><strong>{print.name}</strong><span className="variant-source">Arte seleccionada</span></div></div> : <p className="muted variant-unused">Carga el arte seleccionado para editar su estampado.</p>}
    ...
    <button className="secondary small" onClick={resetPrint}><RotateCcw size={13} /> Restablecer posición</button>
  </div>
}
