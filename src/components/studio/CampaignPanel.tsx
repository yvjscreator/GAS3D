...
export function CampaignPanel() {
  ...
  const activeZone = active ? role === 'main' ? active.zoneAdjustment : active.companionZoneAdjustment : null
  ...
  const updateZone = (value: Partial<NonNullable<typeof activeZone>>) => { if (!active || !activeZone) return; studio.updateCollectionItem(active.id, role === 'main' ? { zoneAdjustment: { ...active.zoneAdjustment, ...value } } : { companionZoneAdjustment: { ...active.companionZoneAdjustment, ...value } }) }
    ...
    {studio.campaignMode === 'variants' ? <div className="campaign-variants"><VariantsPanel /></div> : <>
      ...
      <MasterDetailLayout className="collection-workspace" master={<>
        ...
        <div className="collection-list">{studio.collectionItems.map((item, index) => <article key={item.id} draggable onDragStart={() => setDraggingId(item.id)} onDragEnd={() => setDraggingId(null)} onDragOver={(event) => event.preventDefault()} onDrop={(event) => dropOn(event, item.id)} className={item.id === studio.activeCollectionItemId ? 'collection-card active' : 'collection-card'} onClick={() => selectItem(item, role)}>
          <GripVertical size={14} /><span className="collection-index">{index + 1}</span><span className="collection-pair-thumbs"><button className={`${item.asset.name ? 'ready ' : ''}${item.id === studio.activeCollectionItemId && role === 'main' ? 'selected' : ''}`} onClick={(event) => { event.stopPropagation(); if (item.asset.url) selectItem(item, 'main'); else requestUpload(item, 'main') }} onDoubleClick={(event) => { event.stopPropagation(); requestUpload(item, 'main') }} onKeyDown={(event) => { if (event.key === 'Enter' && item.asset.url) { event.preventDefault(); requestUpload(item, 'main') } }} title="Clic: seleccionar · doble clic o Enter: cargar o reemplazar" aria-label={`${item.asset.url ? 'Seleccionar estampado principal; presiona Enter para reemplazar' : 'Cargar estampado principal'}`}>{item.asset.url ? <img src={item.asset.thumbnailUrl || item.asset.url} alt="" /> : <ImagePlus size={14} />}<b>P</b></button><button className={`${item.companionAsset.name ? 'ready ' : ''}${item.id === studio.activeCollectionItemId && role === 'companion' ? 'selected' : ''}`} onClick={(event) => { event.stopPropagation(); if (item.companionAsset.url) selectItem(item, 'companion'); else requestUpload(item, 'companion') }} onDoubleClick={(event) => { event.stopPropagation(); requestUpload(item, 'companion') }} onKeyDown={(event) => { if (event.key === 'Enter' && item.companionAsset.url) { event.preventDefault(); requestUpload(item, 'companion') } }} title="Clic: seleccionar · doble clic o Enter: cargar o reemplazar" aria-label={`${item.companionAsset.url ? 'Seleccionar companion; presiona Enter para reemplazar' : 'Cargar companion'}`}>{item.companionAsset.url ? <img src={item.companionAsset.thumbnailUrl || item.companionAsset.url} alt="" /> : <ImagePlus size={14} />}<b>C</b></button></span><span className="collection-card-copy"><strong>{item.name}</strong><small>{isCompleteCollectionItem(item) ? `Grupo ${presentationGroups.findIndex((group) => group.some((candidate) => candidate.id === item.id)) + 1} · Par completo` : 'Falta completar el par'}</small></span><span className="collection-card-actions"><IconButton icon={ChevronUp} label="Subir diseño" disabled={index === 0} onClick={(event) => { event.stopPropagation(); studio.moveCollectionItem(item.id, -1) }} /><IconButton icon={ChevronDown} label="Bajar diseño" disabled={index === studio.collectionItems.length - 1} onClick={(event) => { event.stopPropagation(); studio.moveCollectionItem(item.id, 1) }} /><IconButton icon={Trash2} label="Eliminar diseño" className="danger" onClick={(event) => { event.stopPropagation(); removeItem(item) }} /></span>
        </article>)}</div>
      </>} detail={active && activePrint && activeZone && activePlacement && activeAsset ? <div className="collection-inspector">
        ...
        <PrintZoneInspector placement={activePlacement} print={{ ...activePrint, url: activeAsset.url, name: activeAsset.name }} zoneAdjustment={activeZone} editorMode={studio.editorMode} onEditorModeChange={studio.setEditorMode} onUpdatePrint={updatePrint} onUpdateZone={updateZone} onAlign={(alignment, target) => studio.alignPrint(activePlacement, alignment, target)} />
