...
export function BackgroundPanel() {
  ...
  const range = (label: string, key: 'blur' | 'darkness' | 'ambilightStrength' | 'ambilightReach') => <label className="range-row">{label}<output>{background[key]}%</output><input type="range" min="0" max="100" value={background[key]} onChange={(e) => setBackground({ [key]: Number(e.target.value) })} /></label>
  return <section className="panel"><h2>Fondo</h2><div className="tabs">{(['color', 'image', 'video'] as const).map((type) => <button key={type} className={background.type === type ? 'selected' : ''} onClick={() => { setBackground({ type }); setError(null) }}>{type === 'color' ? 'Color' : type === 'image' ? 'Imagen' : 'Video'}</button>)}</div>
    ...
    {background.type !== 'color' && range('Desenfoque', 'blur')}{range('Oscurecer fondo', 'darkness')}
