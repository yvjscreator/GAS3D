...
export function AdvancedDirectorPanel({ framing, draft, onBeginFraming, onCancelFraming, onSaveFraming, onResetFraming, onDraftFov, onDraftComposition }: {
  ...
  const participatingItems = studio.collectionItems.filter(isCompleteCollectionItem)
  const participatingCombinations = studio.designCombinations.filter((item) => item.enabled).sort((a, b) => a.order - b.order)
  ...
  const directionMaster = <div className="direction-master"><div className="advanced-section-title"><Rotate3D size={14} /><span>Tipo de director</span></div>
    ...
    <ResponsiveOptionGrid minWidth={170} className="director-shot-grid">{visibleShots.map((shot) => { const Icon = shotIcons[shot.id]; return <label key={shot.id} className="collection-option-card"><input type="checkbox" checked={studio.enabledShotTypes.includes(shot.id)} onChange={() => studio.toggleShotType(shot.id)} /><Icon size={16} /><span><strong>{shot.name}</strong><small>{shot.description}</small></span></label> })}</ResponsiveOptionGrid>
    {!visibleShots.some((shot) => studio.enabledShotTypes.includes(shot.id)) && <p className="advanced-current-variant">Activa al menos una toma aplicable para construir la película.</p>}
  ...
  const directionInspector = <div className="direction-inspector"><div className="advanced-section-title"><Frame size={14} /><span>{collectionMode ? 'Encuadre por diseño' : 'Encuadre por combinación'}</span></div>
    <div className="framing-actions">
      {!framing ? <><span className="framing-summary">{draft.saved ? <><Check size={13} /> Encuadre personalizado</> : 'Encuadre predeterminado'}</span><button className="primary-icon-button" disabled={!collectionItem && !designCombination} onClick={onBeginFraming}><Frame size={14} /> Editar</button>{draft.saved && <button onClick={onResetFraming} title="Volver al encuadre predeterminado"><RotateCcw size={14} /> Restablecer</button>}</> : <><button className="save-framing" onClick={() => onSaveFraming()}><Save size={14} /> Guardar</button><button onClick={onCancelFraming}>Cancelar</button></>}
    </div>
    {framing && <><label className="range-row">Campo de visión<output>{draft.fov.toFixed(0)}°</output><input type="range" min="22" max="55" value={draft.fov} onChange={(event) => onDraftFov(Number(event.target.value))} /></label><div className="composition-control"><span>Posición en el cuadro</span><div className="composition-grid">{alignmentPoints.map(([x, y]) => <button key={`${x}-${y}`} className={draft.composition[0] === x && draft.composition[1] === y ? 'active' : ''} onClick={() => onDraftComposition([x, y])} aria-label={`Encuadre ${x}, ${y}`}><i /></button>)}</div></div></>}
    {label && <><div className="advanced-section-title label-title"><span>Etiqueta diferenciadora</span><span className="label-title-actions"><label className="inline-switch"><input type="checkbox" checked={label.enabled} onChange={(event) => updateLabel({ enabled: event.target.checked })} />{label.enabled ? <Eye size={13} /> : <EyeOff size={13} />} Mostrar</label><button className="label-menu-trigger" onClick={() => setLabelMenuOpen((open) => !open)} aria-label="Más acciones de etiquetas"><EllipsisVertical size={14} /></button></span></div>
    {labelMenuOpen && <div className="label-batch-menu"><button onClick={() => { setAllLabels(true); setLabelMenuOpen(false) }}><Eye size={13} /> Activar todas</button><button onClick={() => { setAllLabels(false); setLabelMenuOpen(false) }}><EyeOff size={13} /> Desactivar todas</button></div>}
    {label.enabled && <>
    ...
    <div className="layer-inline"><label className="inline-switch"><input type="checkbox" checked={label.borderEnabled} onChange={(event) => updateLabel({ borderEnabled: event.target.checked })} /> Borde</label>{(label.backgroundEnabled || label.borderEnabled) && <label>Radio px<input type="number" min="0" max="40" value={label.borderRadius} onChange={(event) => updateLabel({ borderRadius: Number(event.target.value) })} /></label>}</div>
