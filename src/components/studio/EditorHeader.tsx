...
export function EditorHeader({ canUndo, canRedo, onUndo, onRedo, renderLayers, renderSettings, layerCount, playing, recording, previewDisabled, recordDisabled, onPreview, onRecord }: Props) {
  ...
  return <header className="editor-header">
    <ToolGroup label="Edición" className="editor-history-tools">
      ...
      <Popover open={layersOpen} onOpenChange={(next) => { setLayersOpen(next); if (next) setSettingsOpen(false) }} placement="viewport" width={380} className="layers-popover" trigger={({ open, toggle }) => <IconButton icon={Layers3} label={open ? 'Cerrar capas' : 'Abrir capas'} text={`Capas${layerCount ? ` ${layerCount}` : ''}`} aria-expanded={open} onClick={toggle} />}>{renderLayers(() => setLayersOpen(false))}</Popover>
      <Popover open={settingsOpen} onOpenChange={(next) => { setSettingsOpen(next); if (next) setLayersOpen(false) }} placement="viewport" width={560} className="settings-popover" trigger={({ open, toggle }) => <IconButton icon={Settings2} label={open ? 'Cerrar configuración' : 'Abrir configuración'} text="Configuración" aria-expanded={open} onClick={toggle} />}>{renderSettings(() => setSettingsOpen(false))}</Popover>
    </ToolGroup>
