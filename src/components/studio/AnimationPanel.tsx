...
export function AnimationPanel() {
  const { animation, setAnimation, duration, setDuration, setTargetRotation, variantAssets, beatSync } = useStudioStore(); const professional = Boolean(variantAssets.large.url && variantAssets.small.url); const rhythmic = professional && hasBeatMap(beatSync)
  ...
  return <section className="panel"><h2>Animación</h2>
    {professional && <p className="director-note"><strong>Dirección profesional activa</strong><span>4 giros de variante → toma hero → acercamiento principal → detalle secundario.</span></p>}
