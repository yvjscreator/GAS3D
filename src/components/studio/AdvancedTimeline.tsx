...
export function AdvancedTimeline({ playing, onTogglePlay, onSeek, embedded = false, collapsed: controlledCollapsed, onCollapsedChange }: AdvancedTimelineProps) {
  ...
  const canSplit = Boolean(selected && !selected.track.locked && project.playhead > selected.clip.start + .1 && project.playhead < selected.clip.start + selected.clip.duration - .1)
  const selectClipContext = (item: TimelineClip) => {
    studio.selectTimelineClip(item.id)
    const entityId = item.collectionItemId ?? item.designCombinationId ?? item.itemIds?.[0]
    if (!entityId) return
    if (studio.campaignMode === 'collection' && studio.collectionItems.some((candidate) => candidate.id === entityId)) studio.setActiveCollectionItemId(entityId)
    else if (studio.designCombinations.some((candidate) => candidate.id === entityId)) studio.setActiveDesignCombinationId(entityId)
  }
  const beginClipDrag = (event: ReactPointerEvent, track: TimelineTrack, item: TimelineClip, mode: 'move' | 'trimLeft' | 'trimRight' | 'fadeIn' | 'fadeOut') => {
    event.preventDefault(); event.stopPropagation(); selectClipContext(item)
  ...
  return <section className={`advanced-timeline${embedded ? ' embedded' : ''}${collapsed ? ' collapsed' : ''}`} style={embedded ? undefined : { height: collapsed ? 42 : height }}>
    ...
    <div className="timeline-toolbar">
      ...
      {canSplit && selected && <button onClick={() => studio.splitTimelineClip(selected.track.id, selected.clip.id, project.playhead)} title="Dividir en el playhead"><Scissors size={14} /> Dividir</button>}
