...
export function VariantsPanel() {
  ...
  return <section className="panel variants-panel"><h2>Diseño único</h2>
    ...
    <MasterDetailLayout className="single-design-workspace" master={<div className="single-design-master">
      ...
      <div className="single-variant-fields"><label className="layer-field variant-name-field">Nombre<input value={active.name} onChange={(event) => updateName(event.target.value)} /></label><label className="select-row">Foco inicial<select value={active.focusRole} onChange={(event) => studio.updateDesignCombination(active.id, { focusRole: event.target.value as DesignAssetRole })}><option value="main">Principal</option><option value="companion">Companion</option></select></label><label className="collection-color-field">Color de la remera<input type="color" value={active.garmentColor} onChange={(event) => studio.updateDesignCombination(active.id, { garmentColor: event.target.value })} /></label><label className="inline-switch variant-enabled-field"><input type="checkbox" checked={active.enabled} onChange={() => studio.updateDesignCombination(active.id, { enabled: !active.enabled })} /> Incluir en el video</label></div>
      <div className="variant-role-selector">{(['main', 'companion'] as DesignAssetRole[]).map((role) => { const assetRole = role === 'main' ? 'large' : 'small'; const asset = studio.variantAssets[assetRole]; const placement = role === 'main' ? active.mainPlacement : active.companionPlacement; return <button key={role} className={selectedRole === role ? 'active' : ''} onClick={() => selectRole(assetRole)}>{(asset.thumbnailUrl || asset.url) ? <img src={(asset.thumbnailUrl || asset.url)!} alt="" /> : <ImagePlus size={17} />}<span><strong>{role === 'main' ? 'Principal' : 'Companion'}</strong><small>{asset.name ?? 'Sin arte'} · {placementLabels[placement]}</small></span><i /></button> })}</div>
