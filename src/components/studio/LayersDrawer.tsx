...
import { EllipsisVertical, Image as ImageIcon, Layers3, Lock, Music2, Plus, RefreshCw, Type, X } from '../icons'
  ...
  return <aside className={`${visible ? 'layers-drawer open' : 'layers-drawer'}${embedded ? ' embedded' : ''}`}>
    ...
    {visible && <div className="layers-content">
      ...
      {(selectedSystem || selectedOverlay || selectedMusic) && <div className="layer-options">
        <button className="layer-options-title" onClick={() => setOptionsOpen(!optionsOpen)}><span>Opciones de la capa</span><b>{optionsOpen ? '−' : '+'}</b></button>
