...
import { getProfessionalDuration } from '../../config/professionalRecording'
...
export function BeatSyncPanel() {
  ...
  const analyze = async () => {
    ...
    try {
      const key = studio.beatSync.source === 'music' ? musicMediaKey : backgroundMediaKey
      const blob = await loadMedia(key)
      if (!blob || !sourceName) throw new Error('Primero carga el audio que quieres analizar.')
      const result = await analyzeBeatBlob(blob, studio.beatSync.sensitivity)
      studio.setBeatSync({ ...result, enabled: true, analyzedAssetName: sourceName })
      if (studio.beatSync.source === 'music' && studio.music.sourceDuration > studio.music.duration) studio.setMusic({ duration: studio.music.sourceDuration })
  ...
  return <section className="panel beat-sync-panel"><h2>Sincronización musical</h2>
    <label className="toggle-row beat-master"><input type="checkbox" checked={studio.beatSync.enabled} onChange={(event) => studio.setBeatSync({ enabled: event.target.checked })} /><span><strong>Animar al ritmo</strong><small>Funciona con las cuatro variantes y con colecciones.</small></span></label>
    <div className="beat-source-tabs" role="group" aria-label="Audio para analizar"><button disabled={!musicAvailable} className={studio.beatSync.source === 'music' ? 'active' : ''} onClick={() => selectSource('music')}>Música</button><button disabled={!backgroundAvailable} className={studio.beatSync.source === 'background' ? 'active' : ''} onClick={() => selectSource('background')}>Audio del fondo</button></div>
    {!musicAvailable && !backgroundAvailable && <p className="beat-empty">Carga música desde Capas o un fondo de video para detectar automáticamente el ritmo.</p>}
    {studio.beatSync.source === 'background' && backgroundAvailable && !studio.background.videoAudioEnabled && <p className="beat-warning">La animación seguirá este video, pero su audio no se exportará hasta activar “Usar audio del video”.</p>}
    {sourceName && <div className="beat-analysis-card"><AudioLines size={18} /><span><strong>{sourceName}</strong><small>{studio.beatSync.analyzedAssetName && !stale ? `${studio.beatSync.bpm.toFixed(1)} BPM · ${studio.beatSync.beats.length} golpes` : 'Pendiente de análisis'}</small></span><button disabled={analyzing} onClick={() => void analyze()}>{analyzing ? 'Analizando…' : <><RefreshCw size={12} /> Analizar</>}</button></div>}
