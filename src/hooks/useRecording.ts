  ...
  const start = useCallback((args: Args) => {
    ...
    const draw = (seconds: number) => {
      ...
      if (advancedProject) activeLabelClips(advancedProject, seconds).forEach((item) => {
        ...
        context.translate(x, y); context.scale(transition.scale, transition.scale); context.translate(-x, -y); context.fillStyle = settings.backgroundColor; context.globalAlpha *= settings.backgroundOpacity / 100; context.beginPath(); context.roundRect(x - metrics.width / 2 - paddingX, y - fontSize / 2 - paddingY, metrics.width + paddingX * 2, fontSize + paddingY * 2, settings.borderRadius * outputWidth / 1080); context.fill(); context.globalAlpha = clipOpacity(item, seconds) * transition.opacity; context.strokeStyle = settings.borderColor; context.stroke(); context.fillStyle = settings.color; context.fillText(settings.text, x, y); context.restore()
      })
    }
